...
- **隐藏信息流中的推广内容**：基础过滤仍保持保守，只隐藏首页和微博原生“最新微博”中由专用语义标签明确标记为“荐读”的卡片，不扫描普通微博正文或昵称关键词，也不承诺完整去广告。
- **使用强力规则（可能误伤）**：默认关闭，且只在启用基础过滤后生效；可额外尝试识别专用语义标签中的“推荐”“广告”“推广”、TipsAd 风格的明确广告模块和部分强推荐信号，可能存在误伤，也不承诺完整去广告。
- **自动展开原创长微博**：默认关闭；在首页、微博原生“最新微博”和数字 UID 个人主页中，自动展开进入视野的原创长微博；转发微博卡片保持折叠。原生展开可能触发微博自身的正文加载，Toolkit 不直接请求长微博全文。
...
普通原地升级兼容现有 v0.2.0–v0.9.2 本地状态，不需要仅为升级而恢复备份。跨浏览器、跨设备或卸载/重装前建议先导出。
